...
void printTable(int data[][2], int numOfRows)
{
        ...
        char* tableHeader[2] = {"Replacement Algorithm", "Optimal"};
...
int predict(int pageRequest[], int numberOfPages, int freq[], int frames[], int frameSize, int index){
        int res = -1;
        int farthest = index;
        ...
        for (int i = minFrames; i <= maxFrames; i++) {
                ...
                for (int j = 0; j < pagesNum; j++) {
                        ...
                        for (int k = 0; k < numFrames; k++) {
                                if (frameBuffer[k] == pageRequests[j]) {
                                        pageFound = true;
                                        break;
                                }
                        }

                        freq[pageRequests[j]] -= 1;
                        ...
                        if (!pageFound) {
                                ...
                                if(frameBufferSize < numFrames) {
                                        frameBuffer[frameBufferSize] = pageRequests[j];
                                        frameBufferSize += 1;
                                        continue;
                                }

                                replace = predict(pageRequests, pagesNum, freq, frameBuffer, numFrames, j);
                                if(replace > -1)
                                        frameBuffer[replace] = pageRequests[j];
                        }
